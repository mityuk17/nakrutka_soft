    ...
    while targets:
        for i in range(len(targets)):
            ...
            try:
                ...
                ses = SessionManager.from_tdata_folder(Path(fr"sessions/{session[1]}/tdata"))
                ses.api = api
                client = ses.pyrogram_client(proxy=proxy)

                client = Client(f'session_files/my_account',api.api_id, api.api_hash)
